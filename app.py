...
try:
    ...
    # Se omite silenciosamente si no se encuentra el logo, o puedes mostrar un warning
    pass
...
if consulta:
    ...
    if consulta_lower.isdigit():
        resultados = df[
            (
                df["Material"]
                .astype(str)
                .str.contains(consulta_lower, na=False)
            )
            |
            (
                df["Texto breve de material"]
                .astype(str)
                .str.contains(consulta_lower, case=False, na=False)
            )
        ].copy()
    ...
    # -------------------------------------------------
    # CASO 2: BÚSQUEDA POR TEXTO / PALABRAS
    # -------------------------------------------------
    else:
        def contar_coincidencias(texto):
            ...
            return sum(palabra in texto for palabra in palabras)

        # Buscar si coinciden todas las palabras clave ingresadas
        coincidencias_exactas = df[
            df["search_col"].apply(
                lambda x: all(palabra in x for palabra in palabras)
            )
        ].copy()
        ...
        if not coincidencias_exactas.empty:
            ...
            # Búsqueda difusa (Fuzzy Search) si no hay coincidencias exactas
    ...
    if not resultados.empty:
        ...
        resultados["exacto"] = (
            ...
            .apply(lambda x: all(palabra in x.replace("-", " ") for palabra in palabras))
        ).astype(int)
        ...
        resultados["coincidencia_medida_exacta"] = resultados["search_col"].apply(
            lambda x: sum(medida in x.split() for medida in medidas_busqueda)
        )

        # Prioridad por tipo de palabra encontrada (medidas > números > texto)
        ...
        # Criterios jerárquicos de ordenamiento
        ...
        for _, fila in resultados.iterrows():
            ...
            with st.container(border=True):
                ...
                with col7:
                    ...
                    if str(fila["Parte crítica"]).strip().upper() in ["SI", "S", "X", "TRUE"]:
                        st.success("SI")
                    else:
                        st.write("NO")
    else:
